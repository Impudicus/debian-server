...
main() {
    ...
    validateConfiguration "$config_file" || {
        printLog "error" "Lookup failed! Configuration file is invalid!"
        exit 1
    }

    printLog "info" "Current job: Importing configuration file..."
    source "$config_file" || {
        printLog "error" "Lookup failed! Unable to import configuration file!"
        exit 1
    }
    repository=${repository:-$RESTIC_REPOSITORY} || {
        printLog "error" "Lookup failed! Repository not provided!"
        exit 1
    }

    printLog "info" "Current job: Lookup restic repository '$repository'..."
    restic snapshots --repo "$repository" || {
        printLog "error" "Lookup failed! Unable to lookup restic repository!"
        exit 1
    }

    # --------------------------------------------------
    local run_time=$((SECONDS - SCRIPT_TIME))
    printLog "success" "Lookup executed successfully. Run time: $run_time seconds."
    exit 0
}
