...
main() {
    ...
    getContainerRunstate "imagemaid" && {
        printLog "info" "Job skipped. Container 'imagemaid' running."
        exit 1
    }
    ...
    getContainerRunstate "kometa" && {
        printLog "error" "Job failed! Container 'kometa' running!"
        exit 1
    }

    # --------------------------------------------------
    docker start "imagemaid" > /dev/null || {
        printLog "error" "Job failed! Unable to start container 'imagemaid'!"
        exit 1
    }

    while getContainerRunstate "imagemaid"; do
