...
main() {
    ...
    getContainer "nextcloud" || {
        printLog "error" "Migration failed! Container 'nextcloud' not found!"
        exit 1
    }
    getContainerRunstate "nextcloud" || {
        printLog "error" "Migration failed! Container 'nextcloud' not running!"
        exit 1
    }

    # --------------------------------------------------
    docker exec --user www-data nextcloud php occ maintenance:repair --include-expensive || {
        printLog "error" "Migration failed! Error while executing mimetype migration!"
        exit 1
    }

    # --------------------------------------------------
    local run_time=$((SECONDS - SCRIPT_TIME))
    printLog "success" "Migration executed successfully. Run time: $run_time seconds."
    exit 0
}
