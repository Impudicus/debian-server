...
# scan new user-folders
container_exec "nextcloud" "php occ files:scan --all" "www-data"
if [ $? -ne 0 ]; then
    notification "error" "job failed (error while running command)!"
    exit 1
fi

# scan new group-folders
container_exec "nextcloud" "php occ groupfolders:scan 1" "www-data"
if [ $? -ne 0 ]; then
    notification "error" "job failed (error while running command)!"
    exit 1
fi

container_exec "nextcloud" "php occ groupfolders:scan 2" "www-data"
if [ $? -ne 0 ]; then
    notification "error" "job failed (error while running command)!"
    exit 1
fi
